...
def news_retriever(state: AppState):
    ...
    # Save as CSV
    csv_path = os.path.join(save_dir, f"{ticker}_news.csv")
    news_df.to_csv(csv_path, index=False)

    # Save as JSON
    json_path = os.path.join(save_dir, f"{ticker}_news.json")
    news_df.to_json(json_path, orient="records", indent=4)

    return {"news": news_df, "csv_saved": csv_path, "json_saved": json_path}
...
def price_retriever(state: AppState):
    ...
    # Move the cutoff point 1 month + 5 days back
    cutoff_date = price_df.index[-1] - timedelta(weeks=4, days=5)

    # Filter data to stop at the adjusted date
